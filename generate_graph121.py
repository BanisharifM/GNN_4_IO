...
def load_and_process_data(train_file_path, total_rows, yaml_file_path, test_size=0.25, val_size=0.1, log_interval=10000):
    ...
    logging.info(f"Splitting training data into train and validation sets with validation size = {val_size}")
    train_data, val_data = train_test_split(train_data, test_size=val_size, random_state=1)
    
    logging.info(f"Dataset split into {len(train_data)} training rows, {len(val_data)} validation rows, and {len(test_data)} testing rows.")
    ...
    # Save validation graph data and tags
    logging.info(f"Processing validation data with {len(val_data)} rows.")
    val_graph_file = os.path.join(BASE_DIR, 'val_graphs.csv')
    val_tag_file = os.path.join(BASE_DIR, 'val_tags.csv')
    save_graph_data(val_graph_file, val_tag_file, val_data, total_rows, graph_structure)

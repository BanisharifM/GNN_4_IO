...
def save_graph_data(output_dir, output_file, data, total_rows, graph_structure):
    ...
    tags_df.to_csv(os.path.join(output_dir, 'tags.csv'), mode='a', header=not os.path.exists(os.path.join(output_dir, 'tags.csv')), index=False)
    logging.info(f"All graphs and tags saved to {output_dir}.")
...
yaml_file_path = "Graphs/Graph110/graph_structure.yaml"

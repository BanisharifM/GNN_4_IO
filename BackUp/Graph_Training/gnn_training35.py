...
from torch_geometric.nn import GCNConv, global_mean_pool, GATConv, GraphSAGE
...
train_file_path = "Graphs/Graph35/train_graphs.csv"
val_file_path = "Graphs/Graph35/val_graphs.csv"
...
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join('Graphs/Graph35', 'best_model.pt'))
...
train_losses, val_losses, train_accuracies, val_accuracies = train_and_validate(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(os.path.join('Graphs/Graph35', 'loss_chart.png'))
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig(os.path.join('Graphs/Graph35', 'accuracy_chart.png'))

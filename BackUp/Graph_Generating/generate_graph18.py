...
def generate_graph(yaml_path, train_path, val_path, test_path, save_path):
    ...
    def process_nodes(df, split):
        ...
        for index, row in df.iterrows():
            ...
            # Primary nodes
            for primary_node in graph_structure[tag_node].keys():
    ...
    for index in range(len(train_df)):
        for tag_node, primary_nodes in graph_structure.items():
            for primary_node in primary_nodes.keys():
                edges_data.append({"index": index, "source": tag_node, "target": primary_node})

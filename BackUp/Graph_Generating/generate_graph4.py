...
def generate_graph(yaml_path, data_path, save_path):
    ...
    with h5py.File(os.path.join(save_path, 'graph_data.h5'), 'w') as f:
        ...
        edge_index_obj_array = np.empty(len(edge_index_list), dtype=h5py.vlen_dtype(np.dtype('int32')))
        edge_index_obj_array[:] = edge_index_list
        
        features_obj_array = np.empty(len(features_list), dtype=h5py.vlen_dtype(np.dtype('float32')))
        features_obj_array[:] = features_list
        
        # Save variable-length edge_index
        edge_index_ds = f.create_dataset('edge_index', data=edge_index_obj_array, chunks=True, compression='gzip')
        
        # Save variable-length features
        features_ds = f.create_dataset('features', data=features_obj_array, chunks=True, compression='gzip')
